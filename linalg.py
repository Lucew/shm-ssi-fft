...
def get_block_hankel_representation(time_series: np.ndarray, end_index: int, window_length: int,
                                    window_number: int) -> tuple[np.ndarray, int]:
    """
    Accepted input shapes:
    - (T, m):    legacy input, interpreted as blocks of shape (m, 1)
    - (T, m, n): block input, interpreted as blocks of shape (m, n)

    Returns shape:
        (window_length + window_number - 1, m, n)
    """
    if time_series.ndim == 2:
        # Backwards-compatible mx1 case.
        time_series = time_series[..., None]
    elif time_series.ndim != 3:
        raise ValueError(
            "Input time_series must have shape (T, m) or (T, m, n). "
            f"Currently: {time_series.shape=}."
        )

    size = window_length + window_number - 1

    if not 0 <= end_index <= time_series.shape[0]:
        raise ValueError(
            f"end_index={end_index} is outside the available range [0, {time_series.shape[0]}]."
        )
    if end_index < size:
        raise ValueError(
            f"Need at least {size} samples before end_index, but got end_index={end_index}."
        )

    return time_series[end_index - size:end_index, :, :], size
...
class BlockHankelRepresentation(BlockHankel):
    ...
    def __init__(self, time_series: np.ndarray, end_index: int, window_length: int, window_number: int,
        ...
        _, m, n = hankel.shape
        self.hankel = hankel
        self.shape = (self.window_length * m, self.window_number * n)
...
class BlockHankelProductRepresentation(BlockHankel):
    ...
    def __init__(self, first_hankel: BlockHankel, second_hankel: BlockHankel):
        ...
        if self.first_hankel.shape[1] != self.second_hankel.shape[0]:
            raise ValueError(f'matmul: Operand shape missmatch: {self.first_hankel.shape} @ {self.second_hankel.shape} missmatch in inner dimensions.')
...
def main():
    """
    Example code that creates a block Hankel matrix and tests different matrix operations such as multiplication and
    transpose.

    These are important as they are the main operations used in rSVD for Matrix decomposition.
    :return:
    """
    import timeit
    ...
    m = 10  # rows per block
    r = 10  # columns per block
    s = 15  # rows of left multiplier A

    # make a sequence of block matrices of the correct shape
    block_hankel_sequence = np.arange(0, (p + q - 1) * m * r)
    block_hankel_sequence = block_hankel_sequence.reshape(p + q - 1, m, r)

    hankel_direct = BlockHankelRepresentation(block_hankel_sequence, p + q - 1, p, q, fast_hankel=False)
    hankel_fft = BlockHankelRepresentation(block_hankel_sequence, p + q - 1, p, q, fast_hankel=True)
    hankel_naive = hankel_direct.materialize()

    right_matrix = np.arange(0, hankel_naive.shape[1] * s)
    right_matrix = right_matrix.reshape(hankel_naive.shape[1], s)
    ...
    run_num = 100
    fft_time = timeit.timeit(lambda: BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=True) @ right_matrix, number=run_num) / run_num * 1000
    print('FFT right multiplication took:', fft_time)
    direct_time = timeit.timeit(lambda: BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=False) @ right_matrix, number=run_num) / run_num * 1000
    print('Direct right multiplication took:', direct_time)
    direct_time = timeit.timeit(lambda: BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=False).materialize() @ right_matrix, number=run_num) / run_num * 1000
    ...
    run_num = 100
    fft_time = timeit.timeit(lambda: left_matrix @ BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=True), number=run_num) / run_num * 1000
    print('FFT left multiplication took:', fft_time)
    direct_time = timeit.timeit(lambda: left_matrix @ BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=False), number=run_num) / run_num * 1000
    print('Direct left multiplication took:', direct_time)
    direct_time = timeit.timeit(lambda: left_matrix @ BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=False).materialize(), number=run_num) / run_num * 1000
    ...
    fft_time = timeit.timeit(
        lambda: np.concatenate((BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=True),
                                BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=True)), axis=1) @ right_matrix,
        number=run_num) / run_num * 1000
    print('FFT right multiplication took:', fft_time)
    direct_time = timeit.timeit(
        lambda: np.concatenate((BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=False),
                                BlockHankelRepresentation(block_hankel_sequence, p+q-1, p, q, fast_hankel=False)), axis=1) @ right_matrix,
        number=run_num) / run_num * 1000
    print('Direct right multiplication took:', direct_time)
    direct_time = timeit.timeit(
    ...
    run_num = 100
    fft_time = timeit.timeit(
        lambda: left_matrix @ np.concatenate((BlockHankelRepresentation(block_hankel_sequence, p + q - 1, p, q, fast_hankel=True),
                                              BlockHankelRepresentation(block_hankel_sequence, p + q - 1, p, q, fast_hankel=True)),
                                             axis=1),
        number=run_num) / run_num * 1000
    print('FFT left multiplication took:', fft_time)
    direct_time = timeit.timeit(
        lambda: left_matrix @ np.concatenate((BlockHankelRepresentation(block_hankel_sequence, p + q - 1, p, q, fast_hankel=False),
                                              BlockHankelRepresentation(block_hankel_sequence, p + q - 1, p, q, fast_hankel=False)),
                                             axis=1),
        number=run_num) / run_num * 1000
    print('Direct left multiplication took:', direct_time)
    direct_time = timeit.timeit(
